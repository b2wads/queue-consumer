...
describe('[INTEGRATION] cosumer and rabbit-driver', () => {
  ...
  describe('when message adapter fails', () => {
    ...
    context(
      'when driver is configured to requeue on failure and to not discard on adapter failure',
      () => {
        ...
        before(async () => {
          ...
          await rabbit.channel.sendToQueue(
            queueFixture,
            Buffer.from(JSON.stringify(messageFixture))
          )
      ...
      before(async () => {
        ...
        await Promise.all(
          messagesFixtures.map(msg => {
            return rabbit.channel.sendToQueue(
              queueFixture,
